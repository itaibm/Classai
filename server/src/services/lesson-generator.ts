...
function loadKnowledge(year: number, subject: string): string {
  ...
  // The knowledge base keeps UK year numbers; our Year N (ages N+5–N+6) is UK Year N+1.
  const ukYear = year + 1;
  const candidates = [path.join(dir, `year-${ukYear}.md`), path.join(dir, `year${ukYear}.md`)];
